---
title: "Differenziali Salariali in Italia: Analisi RACLI"
...
# Carica metadata periodo
meta_racli <- readRDS(here::here("output/racli/metadata.rds"))
anno_min <- meta_racli$anno_min
anno_max <- meta_racli$anno_max
n_anni <- meta_racli$n_anni
```
...
Il mercato del lavoro italiano è caratterizzato da significative eterogeneità territoriali, settoriali e demografiche nella distribuzione salariale. Questo rapporto presenta un'analisi completa dei dati ISTAT-RACLI (Registro Amministrativo Costo Lavoro Imprese) sui salari orari lordi in Italia per il periodo `r anno_min`-`r anno_max`.
...
- **Periodo**: `r anno_min`-`r anno_max` (`r n_anni` anni, frequenza annuale)
...
gap_contratto_ultimo <- readRDS("output/racli/gap_contratto_ultimo.rds")
...
Nel `r anno_max`, il salario mediano orario nazionale si attesta a **`r sprintf("%.2f", sintesi_descrittive$salario_mediano_nazionale_ultimo)` EUR/ora**. Nel periodo `r anno_min`-`r anno_max`, i salari sono cresciuti del **`r sprintf("%.1f%%", sintesi_descrittive$crescita_media)`** complessivamente, con un tasso di crescita annuo medio dello **`r sprintf("%.2f%%", sintesi_regressioni$crescita_annua_pct)`**.

```{r fig-evoluzione, fig.cap=paste0("Evoluzione salario mediano nazionale ", anno_min, "-", anno_max), out.width="90%"}
knitr::include_graphics(here::here("output", "racli", "grafici", "01_evoluzione_salari_nazionale.png"))
```

## Differenziali di Genere

Il gap salariale di genere nel `r anno_max` è pari all'**`r sprintf("%.1f%%", sintesi_descrittive$gap_gender_medio_ultimo)`**, in calo rispetto all'8.8% del `r anno_min`. L'analisi di regressione indica una riduzione del gap di **0.26 punti percentuali all'anno**, statisticamente significativa.
...
gap_gender_ultimo %>%
  ...
  kable(
    ...
    digits = 2,
    caption = paste0("Gap salariale di genere per settore (", anno_max, ")"),
    booktabs = TRUE
  ) %>%
  kable_styling(latex_options = c("hold_position"))
```

```{r fig-gap-gender, fig.cap=paste0("Evoluzione gap di genere ", anno_min, "-", anno_max), out.width="90%"}
knitr::include_graphics(here::here("output", "racli", "grafici", "02_gap_genere_tempo.png"))
```
...
gap_educazione_ultimo %>%
  ...
  kable(
    ...
    digits = 2,
    caption = paste0("Salari per livello educativo (", anno_max, ")"),
    booktabs = TRUE
  ) %>%
  kable_styling(latex_options = c("hold_position"))
```

```{r fig-educazione, fig.cap=paste0("Salari per livello educativo (", anno_max, ")"), out.width="90%"}
knitr::include_graphics(here::here("output", "racli", "grafici", "09_salari_educazione.png"))
```
...
```{r tab-contratto}
gap_contratto_ultimo %>%
  select(contratto, salario_mediano, salario_medio, D9_D1) %>%
  arrange(desc(salario_mediano)) %>%
  kable(
    col.names = c("Tipo Contratto", "Salario Mediano", "Salario Medio", "D9/D1"),
    digits = 2,
    caption = paste0("Salari per tipo contratto (", anno_max, ")"),
    booktabs = TRUE
  ) %>%
  kable_styling(latex_options = c("hold_position"))
```

```{r fig-contratto, fig.cap=paste0("Salari per tipo contratto (", anno_max, ")"), out.width="90%"}
knitr::include_graphics(here::here("output", "racli", "grafici", "10_salari_contratto.png"))
```
...
stat_ripartizioni_ultimo %>%
  ...
  kable(
    ...
    digits = c(0, 2, 2),
    caption = paste0("Statistiche salariali per ripartizione (", anno_max, ")"),
    booktabs = TRUE
  ) %>%
  kable_styling(latex_options = c("hold_position"))
```

```{r fig-ripartizioni, fig.cap=paste0("Salari mediani per ripartizione geografica (", anno_max, ")"), out.width="90%"}
knitr::include_graphics(here::here("output", "racli", "grafici", "03_salari_ripartizioni.png"))
```

```{r fig-boxplot, fig.cap=paste0("Distribuzione salari provinciali per ripartizione (", anno_max, ")"), out.width="90%"}
knitr::include_graphics(here::here("output", "racli", "grafici", "06_boxplot_salari_ripartizioni.png"))
```
...
```{r fig-mappa-salari, fig.cap=paste0("Mappa salari mediani provinciali (", anno_max, ")"), out.width="90%"}
knitr::include_graphics(here::here("output", "racli", "grafici", "21_mappa_salari_province.png"))
```
...
```{r fig-mappa-crescita, fig.cap=paste0("Mappa crescita salariale ", anno_min, "-", anno_max, " (%)"), out.width="90%"}
knitr::include_graphics(here::here("output", "racli", "grafici", "22_mappa_crescita_salari.png"))
```
...
```{r fig-mappa-dispersione, fig.cap=paste0("Mappa dispersione salariale D9/D1 (", anno_max, ")"), out.width="90%"}
knitr::include_graphics(here::here("output", "racli", "grafici", "23_mappa_disuguaglianza_D9_D1.png"))
```
...
```{r fig-mappa-cluster, fig.cap=paste0("Mappa clustering k-means province (", anno_max, ")"), out.width="90%"}
knitr::include_graphics(here::here("output", "racli", "grafici", "24_mappa_cluster_province.png"))
```
...
dispersione_settori <- readRDS("output/racli/dispersione_settori_ultimo.rds")
gap_gender_settori <- readRDS("output/racli/gap_gender_settori_ultimo.rds")
```
...
ranking_settori_nace %>%
  ...
  kable(
    ...
    digits = 2,
    caption = paste0("Top 10 settori per salario mediano (", anno_max, ", EUR/h)"),
    booktabs = TRUE
  ) %>%
...
ranking_settori_nace %>%
  ...
  kable(
    col.names = c("Settore", "Sal. Med.", "Sal. Medio", "D9/D1"),
    digits = 2,
    caption = paste0("Bottom 10 settori per salario mediano (", anno_max, ", EUR/h)"),
    booktabs = TRUE
  ) %>%
  kable_styling(latex_options = c("hold_position"), font_size = 9) %>%
  column_spec(1, width = "6cm")
...
```{r fig-ranking-settori, fig.cap=paste0("Ranking settori NACE per salario mediano (", anno_max, ")"), out.width="90%"}
knitr::include_graphics(here::here("output", "racli", "grafici", "16_salari_settori_nace.png"))
```
...
dispersione_settori %>%
  ...
  kable(
    ...
    digits = 2,
    caption = paste0("Settori con maggiore dispersione salariale (D9/D1, ", anno_max, ", EUR/h)"),
    booktabs = TRUE
  ) %>%
  kable_styling(latex_options = c("hold_position"), font_size = 9) %>%
  column_spec(1, width = "6cm")
...
```{r fig-scatter-settori, fig.cap=paste0("Relazione tra livello salariale e dispersione per settore (", anno_max, ")"), out.width="90%"}
knitr::include_graphics(here::here("output", "racli", "grafici", "17_scatter_salari_dispersione_settori.png"))
```
...
gap_gender_settori %>%
  ...
  kable(
    ...
    digits = c(0, 2, 2, 1),
    caption = paste0("Settori con maggior gap di genere (", anno_max, ", EUR/h)"),
    booktabs = TRUE
  ) %>%
  kable_styling(latex_options = c("hold_position"), font_size = 9) %>%
  column_spec(1, width = "6cm")
...
```{r fig-gap-settori, fig.cap=paste0("Gap salariale di genere per settore NACE (", anno_max, ")"), out.width="90%"}
knitr::include_graphics(here::here("output", "racli", "grafici", "18_gap_genere_settori.png"))
```
...
gap_territoriale_settori <- readRDS("output/racli/gap_territoriale_settori_ultimo.rds")
cv_territoriale_settori <- readRDS("output/racli/cv_territoriale_settori_ultimo.rds")
...
gap_territoriale_settori %>%
  ...
  kable(
    ...
    digits = c(0, 2, 2, 1),
    caption = paste0("Settori con maggiore divario Nord-Sud (", anno_max, ")"),
    booktabs = TRUE
  ) %>%
  kable_styling(latex_options = c("hold_position"), font_size = 9) %>%
...
```{r fig-heatmap-settori, fig.cap=paste0("Salario mediano per settore e ripartizione geografica (", anno_max, ")"), out.width="95%"}
knitr::include_graphics(here::here("output", "racli", "grafici", "19_heatmap_settori_ripartizioni.png"))
```
...
```{r fig-gap-nord-sud, fig.cap=paste0("Gap salariale Nord-Sud per settore NACE (", anno_max, ")"), out.width="90%"}
knitr::include_graphics(here::here("output", "racli", "grafici", "20_gap_nord_sud_settori.png"))
```
...
Il rapporto interdecile D9/D1 a livello nazionale nel `r anno_max` è pari a **`r sprintf("%.2f", sintesi_distributiva$D9_D1_ultimo)`**, indicando che i lavoratori nel 90° percentile guadagnano circa 2.6 volte i lavoratori nel 10° percentile. Questo indicatore è preferito rispetto all'indice di Gini in quanto direttamente calcolabile dai dati RACLI disponibili (che forniscono solo i decili D1, D5, D9) senza necessità di assunzioni parametriche sulla forma della distribuzione.

```{r fig-D9D1, fig.cap="Rapporto interdecile D9/D1", out.width="90%"}
knitr::include_graphics(here::here("output", "racli", "grafici", "05_rapporto_D9_D1.png"))
```
...
```{r fig-fanchart, fig.cap=paste0("Fan chart distribuzione salariale: decili D1, D5, D9 (", anno_min, "-", anno_max, ")"), out.width="90%"}
knitr::include_graphics(here::here("output", "racli", "grafici", "12_fan_chart_decili.png"))
```
...
L'analisi di mobilità basata su matrici di transizione tra quartili (`r anno_min`->`r anno_max`) rivela un'**elevata persistenza**: l'83% delle province rimane nello stesso quartile di partenza, indicando bassa mobilità relativa.
...
knitr::include_graphics(here::here("output", "racli", "grafici", "13_sigma_convergenza.png"))
```

```{r fig-beta, fig.cap="$\\beta$-convergenza: regressione crescita su livello iniziale", out.width="90%"}
knitr::include_graphics(here::here("output", "racli", "grafici", "14_beta_convergenza.png"))
```
...
knitr::include_graphics(here::here("output", "racli", "grafici", "08_cluster_province.png"))
```

```{r fig-topbottom, fig.cap=paste0("Top 10 e Bottom 10 province per salario mediano (", anno_max, ")"), out.width="90%"}
knitr::include_graphics(here::here("output", "racli", "grafici", "15_top_bottom_province.png"))
```
...
diff_settori <- readRDS("output/racli/differenziali_settoriali_stimati.rds")
...
diff_settori %>%
  ...
  mutate(
    ci_lower = premio_pct - 1.96 * se,
    ci_upper = premio_pct + 1.96 * se
  ) %>%
...
interaz_genere <- readRDS("output/racli/interazioni_settore_genere.rds")
...
1. **Stagnazione salariale relativa**: crescita complessiva `r anno_min`-`r anno_max` pari a +6.2% (+0.75% annuo), inferiore all'inflazione cumulata
...
5. **Gap di genere in riduzione**: da 8.8% (`r anno_min`) a 8.0% (`r anno_max`), trend negativo significativo a livello aggregato
...
9. **Bassa mobilità relativa**: 83% province rimane nello stesso quartile salariale `r anno_min`-`r anno_max`
...
- **Periodo breve** (`r n_anni` anni) limita l'analisi di trend di lungo periodo
...
La validazione statistica conferma una correlazione molto forte (r = 0.944, p < 0.001) tra le serie RACLI aggregate a livello nazionale e i dati del modello VECM per il periodo sovrapposto `r anno_min`-`r anno_max`, attestando la robustezza e coerenza delle due fonti informative.

```{r fig-validazione, fig.cap=paste0("Validazione RACLI vs VECM (", anno_min, "-", anno_max, ")"), out.width="90%"}
knitr::include_graphics(here::here("output", "racli", "grafici", "16_validazione_racli_vecm.png"))
```
...
- **$\beta$-convergenza**: regressione OLS del tasso crescita annuo su log(salario iniziale `r anno_min`)
...
**Generato con R Markdown** | Dati: ISTAT-RACLI `r anno_min`-`r anno_max` | Autore: Giampaolo Montaletti (ORCID: 0009-0002-5327-1122)
