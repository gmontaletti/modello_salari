...
La mappa della crescita salariale mostra una distribuzione più eterogenea, con alcune province meridionali che registrano tassi di crescita superiori alla media nazionale, coerentemente con le evidenze di β-convergenza.
...
ranking_settori_nace %>%
  ...
  head(10) %>%
  kable(
    col.names = c("Settore", "Salario Mediano (€/h)", "Salario Medio (€/h)", "D9/D1"),
    digits = 2,
    caption = "Top 10 settori per salario mediano (2022)",
    booktabs = TRUE
  ) %>%
...
ranking_settori_nace %>%
  ...
  kable(
    col.names = c("Settore", "Salario Mediano (€/h)", "Salario Medio (€/h)", "D9/D1"),
    digits = 2,
    caption = "Bottom 10 settori per salario mediano (2022)",
    booktabs = TRUE
  ) %>%
  kable_styling(latex_options = c("hold_position", "scale_down"))
```
...
dispersione_settori %>%
  ...
  head(10) %>%
  kable(
    col.names = c("Settore", "Sal. Med. (€/h)", "D9/D1", "D9/D5", "D5/D1"),
    digits = 2,
    caption = "Settori con maggiore dispersione salariale (D9/D1, 2022)",
    booktabs = TRUE
  ) %>%
  kable_styling(latex_options = c("hold_position", "scale_down"))
```
...
gap_gender_settori %>%
  ...
  kable(
    col.names = c("Settore", "Salario F (€/h)", "Salario M (€/h)", "Gap (%)"),
    digits = c(0, 2, 2, 1),
    caption = "Settori con maggior gap di genere (2022)",
    booktabs = TRUE
  ) %>%
  kable_styling(latex_options = c("hold_position", "scale_down"))
```
...
- **σ-convergenza**: `r ifelse(sintesi_distributiva$convergenza_sigma, "CONFERMATA", "NON CONFERMATA")` - riduzione del coefficiente di variazione tra province
- **β-convergenza**: `r ifelse(sintesi_distributiva$convergenza_beta, "CONFERMATA", "NON CONFERMATA")` - le province con salari iniziali più bassi crescono più velocemente

```{r fig-sigma, fig.cap="σ-convergenza: coefficiente di variazione salari provinciali", out.width="90%"}
knitr::include_graphics("output/grafici/13_sigma_convergenza.png")
```

```{r fig-beta, fig.cap="β-convergenza: regressione crescita su livello iniziale", out.width="90%"}
...
| Genere (maschi vs femmine) | +8.0% | In riduzione (-0.26 p.p./anno) |
| Geografia (Nord-Ovest vs Sud) | +13% | Divario persistente |
...
2. **Gap di genere in riduzione**: da 8.8% (2014) a 8.0% (2022), trend negativo significativo
3. **Forte premio educativo**: laureati guadagnano +21% in media, con eterogeneità territoriale elevata
4. **Divario territoriale persistente**: differenziale Nord-Sud del 13%, con evidenze di convergenza lenta
5. **Dispersione salariale stabile**: rapporto D9/D1 nazionale sostanzialmente invariato (~2.6), con significativa eterogeneità tra aree
6. **Bassa mobilità relativa**: 83% province rimane nello stesso quartile salariale 2014-2022
7. **Clustering duale**: mercato del lavoro bipartito (alto/basso reddito) con caratteristiche strutturali distinte
...
- **Aggregazione settoriale** maschera eterogeneità intra-settore (es. qualifiche, mansioni)
...
- **σ-convergenza**: riduzione dispersione misurata tramite coefficiente di variazione
- **β-convergenza**: regressione OLS del tasso crescita annuo su log(salario iniziale 2014)
